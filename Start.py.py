...
import sys
init(autoreset=True)
__version__ = "Ro7 Discord UserName Checker V1"
...
def sys_c_t():
   if configur.get("sys","TOKEN") != "":
      pass
       ...
       exit()
available_usernames = []
av_list = os.path.join(dir_path, f"available_usernames")
sample_0 = r"_."
...
def main():
    ...
    print(f"""{Fore.WHITE}
                                            ...
                                            {Fore.LIGHTBLACK_EX}Connected as {requests.get(sys_url,headers=s_sys_h()).json()['username']}{Ly}
                       
{Fore.LIGHTBLACK_EX}1-{Fore.LIGHTBLACK_EX}[{Fore.RED}Generate names and check{Fore.LIGHTBLACK_EX}]{Ly}             
{Fore.LIGHTBLACK_EX}2-{Fore.LIGHTBLACK_EX}[{Fore.RED}Check a specific list{Fore.LIGHTBLACK_EX}]{Ly}             
                         
                                              Config.ini:
                                              {Fore.LIGHTCYAN_EX}Digits: {Fore.GREEN}{sat_digits}{Ly}
   

                                            {Fore.LIGHTBLACK_EX} Ro7 ON TOPPPPPPPP

                                       
 
{Fore.LIGHTCYAN_EX}String: {Fore.GREEN}{sat_string}{Ly}
{Fore.LIGHTCYAN_EX}Punctuation: {Fore.GREEN}{sat_punct}{Ly}
{Fore.LIGHTCYAN_EX}Multi-Token: {Fore.GREEN}{sat_multi_token}{Ly}
{Fore.LIGHTCYAN_EX}Webhook: {Fore.GREEN}{webhook_0}{Ly}
{Fore.LIGHTCYAN_EX}Delay: {Fore.GREEN}{Delay}{Ly}
...
def setdelay():
   global Delay
   d_input = input(f"{Lb}{Ly}(Press Enter){Lb} : ")
...
def proc0():
    m_input = input(f"{Fore.LIGHTBLACK_EX}[{Fore.LIGHTCYAN_EX}PRESS 1{Fore.LIGHTBLACK_EX}]:> {Fore.LIGHTCYAN_EX}").lower()
    if m_input=="exit":
        sys.exit(0)
    if m_input=="":
        proc0()
   ...
   if opt == 2:
    for username in usernames:
       ...
       if json_endpoint.get("taken") is not None:
           ...
           print(f"{Lb}[?]{Fore.RED} Error validating '{username}': {endpoint.json()['message']} |Evo: Make sure you have a valid token.")
   elif opt == 1:
       body = {
           "username": usernames
       }
       ...
       if json_endpoint.get("taken") is not None:
           ...
           print(f"{Lb}[?]{Fore.RED} Error validating '{usernames}': {endpoint.json()['message']} |RO7: Make sure you have a valid token.")
...
def opt2load():
    global av_list
    global dir_path
    list_path = os.path.join(dir_path, f"available_usernames")
    print(f"{Lb}[!]{Ly}Checking 'available_usernames' for a valid list...")
    try:
     ...
     exit()
       ...
       print(f"{Lb}[!]{Fore.RED} Error: Couldn't find the list (available_usernames). Please make sure to create a valid list file in the same directory: \n({dir_path}\\)")
       exit()
def opt1load():
...
def ch_send_webhook(val0:str):
   if webhook_0 == True:
    ...
    try:
     webhook.post(
       username="روح",
       avatar_url="https://cdn.discordapp.com/attachments/1278112709916889120/1279293946995937394/other_4.png?ex=66d3ead5&is=66d29955&hm=fd8c82d7a5e679bb8ca44871c8b5127c69bfabbe2c8ea83efccb79c7f529e4d1&",
       embeds=[
    {
      ...
      "author": {
        "name": "DONE",
        "url": "https://discord.gg/wPkhtj4yBP",
        "icon_url": "https://cdn.discordapp.com/emojis/1043423071777865789.webp?size=96&quality=lossless"
      },
      "thumbnail": {
        "url": "https://cdn.discordapp.com/attachments/1278112709916889120/1279293946995937394/other_4.png?ex=66d3ead5&is=66d29955&hm=fd8c82d7a5e679bb8ca44871c8b5127c69bfabbe2c8ea83efccb79c7f529e4d1&"
      },
      "fields": [],
      "color": 16768000
    }
       ...
       print(f"{Lb}[!]{Fore.RED} Error: Something went wrong while sending the webhook request. Exception: {s} | ro7: Make sure you have a valid webhook URL")
   else:
      return
def opt1func(v1,v2):
   for i in range(v1):
